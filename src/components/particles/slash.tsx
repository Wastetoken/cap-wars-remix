...
/** Pool name for a class's slash sparks — one permanent pool per class.
 *  Remounting a single keyed pool on character switch disposes GPU buffers
 *  that the WebGPU compute pipeline still references, freezing the canvas
 *  ("Buffer used in submit while destroyed"). */
export const slashSparksName = (id: CharacterId) => `sparks-${id}` as ParticleType
...
/** Per-class slash tint — mutated live on character switch (see Caps.tsx) */
export const slashColorBase = uniform(new THREE.Color('#ffa808'))
export const slashColorGlow = uniform(new THREE.Color('#8f9aff'))
...
export const Slash = () => {
  ...
  const slashNodes = ({ progress }) => {
    ...
    const color1 = slashColorBase.mul(20)
    const glow = slashColorGlow.mul(30)
    ...
    // Length-safe normalize: at the exact screen-center pixel vUv-(0.5,0.5)
    // is (0,0), normalize() → 0/0 → NaN, and the bloom blur pyramid smears a
    // single NaN pixel into fullscreen color garbage during heavy combat.
    const distortVec = vUv.sub(vec2(0.5, 0.5))
    const distortDir = distortVec.div(distortVec.length().max(1e-4))
  ...
  return (
    <>
      ...
      {/* One permanently-mounted spark pool per class — keyed remounts of a
          single pool destroy GPU buffers mid-pipeline and freeze the canvas */}
      {CHARACTER_LIST.map((id) => (
        <VFXParticles
          ...
          size={[0.05, 0.18]}
          fadeSize={[1, 0]}
          colorStart={CHARACTER_VFX[id].slashSparks}
          fadeOpacity={[1, 0]}
          gravity={[0, 0.3, 0]}
          speed={[0, 3]}
          lifetime={[0.5, 1]}
