          ...
          {view === 'register' ? (
            <div id="register-section">
              ...
              <form onSubmit={handleRegister} className="mt-8 space-y-5">
                <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="Email" required className="spell-focus w-full bg-stone-input border border-stone-border rounded-lg py-3 px-4 text-parchment placeholder-ash outline-none transition-all" />
                <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Password" required minLength={6} className="spell-focus w-full bg-stone-input border border-stone-border rounded-lg py-3 px-4 text-parchment placeholder-ash outline-none transition-all" />
                ...
                <button disabled={loading} type="submit" className="cast-btn w-full bg-spell text-void font-bold py-3 rounded-lg hover:bg-spell/90 transition-all disabled:opacity-50">{loading ? 'Creating...' : 'Create account'}</button>
              </form>
            </div>
          ) : (
            <div id="login-section">
              <h2 className="font-display text-3xl font-semibold text-parchment">Log in</h2>
              ...
              <form onSubmit={handleLogin} className="mt-8 space-y-5">
                <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="Email" required className="spell-focus w-full bg-stone-input border border-stone-border rounded-lg py-3 px-4 text-parchment placeholder-ash outline-none transition-all" />
                <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="Password" required className="spell-focus w-full bg-stone-input border border-stone-border rounded-lg py-3 px-4 text-parchment placeholder-ash outline-none transition-all" />
                {errorMsg && <div className="p-3 bg-red-900/30 border border-red-500/50 rounded-lg text-center text-sm text-red-300">{errorMsg}</div>}
                <button disabled={loading} type="submit" className="cast-btn w-full bg-spell text-void font-bold py-3 rounded-lg hover:bg-spell/90 transition-all disabled:opacity-50">{loading ? 'Logging in...' : 'Log in'}</button>
              </form>
            </div>
          )}
